...
default_path         = get_terrain_related_data("ABRIS_data_path") or lfs.writedir().."PersistentAbris/ABRIS/"
...
function save_file(name,table_to_save,file_to_save)
    ...
    local file = io.open(lfs.writedir().."PersistentAbris/"..file_to_save,'w')
    if file then
        file:write(str)
        file:close()
    end
...
function save_db_file(loader,name,table_to_save,file_to_save)
    ...
    else              path = "ABRIS/Database/"..path end
